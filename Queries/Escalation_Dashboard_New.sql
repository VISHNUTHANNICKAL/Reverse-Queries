...
SELECT [loan_skey],alert_skey
  ...
  --select * from #tempTbl where loan_skey = '258952';
...
from
(
select distinct a.*,b.created_date as 'Intake Date',b.[Intake by],'Comment' as 'Source'
	...
	from #tempTbl a
	join
	(select loan_skey,created_date,created_by as 'Intake by' from  [ReverseQuest].[rms].[v_Note]
	where note_type_description = 'Specialized Escalation Review'
	) b
	on a.loan_skey=b.loan_skey 
	where
	 b.created_date>a.[Assignment date] 
	 -- a.loan_skey = '349090' order by b.created_date
	union
	select a.*,b.modified_date as 'Intake Date',b.modified_by as 'Intake By','Alert' as 'Source'
	...
	where b.modified_date>a.[Assignment date]
	 )x
	group by x.loan_skey,x.[Assignment date],x.[Assignment Note],x.[Assignment By],x.Channel,[Intake by],x.alert_skey
	--,x.Source
	;

	--select distinct * from #tempTbl1 where loan_skey = '258952' order by [Intake Date];
	...
	left join
	#tempTbl1 y on 
	--x.alert_skey=y.alert_skey
	x.loan_skey=y.loan_skey
...
/*	select * from (
	...
	,min([Intake Date]) as [Intake Date],alert_skey into #tempTbl3
	from 
	(select * from (
	select ROW_NUMBER() over (partition by alert_skey order by [Intake Date]) rn,* from #tempTbl1 ) q
	where rn=1 ) p
	...
	--select * from #tempTbl7 where loan_skey = '25579'
	...
	and [Date] <   DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-0, 0)
	order by Date;
	...
	--select distinct * from #tempTbl2 where alert_skey = '33833344';

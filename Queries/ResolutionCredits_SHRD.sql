...
'Jerrick Natividad','Jerrick.Natividad')
	...
	on a.loan_skey=c.loan_skey
  where datediff(day,c.created_date,a.complete_date) > 0 and datediff(day,c.created_date,a.complete_date)  <=120
  and c.created_by not in ('RMS.RepaymentPlan' , 'System.Default') 
;
...
SELECT [loan_skey],alert_skey
  ...
  and created_date  >=   DATEADD(m,-1,DATEADD(mm, DATEDIFF(m,0,GETDATE()), 0))   -45
...
/*	select * from (
	...
	into #rtempTbl7
	from #rtempTbl6
	--where [intake date] > DATEADD(m,-1,DATEADD(mm, DATEDIFF(m,0,GETDATE()), 0)) 
	--and [intake date] < DATEADD(m,0,DATEADD(mm, DATEDIFF(m,0,GETDATE()), 0)) 
	;
	...
	from  #rtempTbl7 a
	where a.[Intake Date] > DATEADD(m,-1,DATEADD(mm, DATEDIFF(m,0,GETDATE()), 0)) 
	and a.[intake date] < DATEADD(m,0,DATEADD(mm, DATEDIFF(m,0,GETDATE()), 0)) 
	;

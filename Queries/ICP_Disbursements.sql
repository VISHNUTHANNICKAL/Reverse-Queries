...
SELECT  [loan_skey]
 ...
 and created_date >=  
 dateadd(m,-3,dateadd(mm,datediff(m,0,getdate()),0 ))
and created_date <    dateadd(m,-2,dateadd(mm,datediff(m,0,getdate()),0 ))
union
...
SELECT  [loan_skey]
  ...
  --and alert_status_description = 'Active'
  and created_date >=  
DATEADD(m,-2,DATEADD(mm, DATEDIFF(m,0,GETDATE()), 0))
and created_date <=
DATEADD(d,-2,DATEADD(mm, DATEDIFF(m,0,GETDATE()),0));
...
and created_date <=
 DATEADD(d,-1,DATEADD(mm, DATEDIFF(m,0,GETDATE()),0));

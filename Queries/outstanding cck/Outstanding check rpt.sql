...
SELECT distinct a.[loan_skey]
      ...
      ,a.[voided_date]
      ,a.[disbursed_date]
      ,a.[payment_stopped_date]
      ,a.[cleared_date]
	  ,DATEDIFF(DAY,a.disbursed_date, GETDATE()) 'age'
	  ,a.disbursement_type_description--,a.transaction_code
	  , case when a.transaction_code=1350 then 'Disb - Unscheduled from LOC'
when a.transaction_code=2650 then 'Disb - Refund Remittance Overage'
when a.transaction_code=1340 then 'Disb - Unscheduled ERNPL LOC'
when a.transaction_code=1250 then 'Disb - Scheduled'
when a.transaction_code=6551 then 'Loss Draft Final - Disb'
when a.transaction_code=1751 then 'Disb - Repair Set Aside Final'
	  else a.transaction_description end as transaction_description
  FROM [ReverseQuest].[rms].[v_Disbursement] a
  where a.[voided_date] is NULL
  and a.[payment_stopped_date] is NULL
  and a.cleared_date is NULL
  and a.disbursement_type_description not like ('%ACH%')
 and a.transaction_code in (
select distinct transaction_code from  [ReverseQuest].[rms].v_Transaction 
 where long_transaction_description in
 ('Disb - Unscheduled ERNPL LOC','Disb - Unscheduled from LOC','Disb - Refund Remittance Overage',
 'Disb - Scheduled' )
 )
 and a.disbursement_type_description in('Refund - Check','Scheduled Payment - Check','Unscheduled Payment - Check')
 union all
 SELECT distinct a.[loan_skey]
      ,a.[pay_to]
      ,a.[check_number]
...
select distinct transaction_code from  [ReverseQuest].[rms].v_Transaction 
 where long_transaction_description in
 ('Disb - Repair Set Aside Final' ) )
 and a.disbursement_type_description in('Repairs - Check')
 union all
 SELECT distinct a.[loan_skey]
      ,a.[pay_to]
      ,a.[check_number]
...
select distinct transaction_code from  [ReverseQuest].[rms].v_Transaction 
 where long_transaction_description in
 ('Loss Draft Final - Disb' ) )
 and a.disbursement_type_description in('Loss Draft - Check')
 ) a
  where
  age between '89' and '150'  
  --loan_skey in ('7307') 
  order by loan_skey;
...
a.loan_status_description in ('Inactive','Active','Claim','Default','Foreclosure')  and
...
and a.loan_skey in(select  loan_skey
from(
SELECT distinct a.[loan_skey]
      ,a.[pay_to]
      ,a.[check_number]
...
-- delete from #rtemp3 where investor_name in('FNMA','MECA 2011')
/*
...
,'Outstanding Checks' as CALLREASON,
...
where a.loan_skey not in (select loan_skey from  #ob12temp 
union
select loan_skey from #ob11temp
--union select loan_skey from #tmpLoankey_1_10_23
)
--loan_status_description in ('ACTIVE','Claim','INACTIVE')

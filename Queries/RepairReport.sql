...
,'Call Request - Cust. Follow-Up','Call Request - COP Change','Call Request - Short Payoff','Call Request-Follow-Up Disb.');
 ...
 IF OBJECT_ID('tempdb..#ib7temp') IS NOT NULL
    DROP TABLE #ib7temp;
SELECT loan_skey into #ib7temp
 from reversequest.rms.v_note a
where a.created_date
>=  DATEADD(d,-4,DATEDIFF(d,0,GETDATE())) and
...
and a.created_by not like 'System Load';
 ...
 IF OBJECT_ID('tempdb..#ib6temp') IS NOT NULL
    DROP TABLE #ib6temp;
SELECT loan_skey into #ib6temp
 from reversequest.rms.v_note a
where a.created_date
>= DATEADD(d,-3,DATEDIFF(d,0,GETDATE())) and
a.created_date < DATEADD(d,0,DATEDIFF(d,0,GETDATE()))
and (a.note_type_description like '%Incom%' or note_type_description like 'Spoc Incom%')
...
IF OBJECT_ID('tempdb..#tmpRepairPopulation') IS NOT NULL
    DROP TABLE #tmpRepairPopulation;
SELECT A.LOAN_SKEY, B.ORIGINAL_LOAN_NUMBER, B.INVESTOR_LOAN_NUMBER, B.SERVICER_NAME, B.INVESTOR_NAME, B.LOAN_POOL_LONG_DESCRIPTION,					
...
FORMAT(A.CREATED_DATE,'d','us') AS CREATED_DATE, A.WORKFLOW_MANAGER, AB.WORKFLOW_TASK_DESCRIPTION,					
FORMAT(AB.SCHEDULE_DATE,'d','us') AS SCHEDULE_DATE, FORMAT(AB.DUE_DATE,'d','us') AS DUE_DATE,					
FORMAT(AB.CREATED_DATE,'d','us') AS CREATED_DATE1, E.REPAIR_SET_ASIDE_AMOUNT AS ORIGINAL_REPAIR_SET_ASIDE, A.REPAIR_SET_ASIDE_AMOUNT,					
...
WHERE A.WORKFLOW_TYPE_DESCRIPTION='REPAIRS'					
AND A.STATUS_SKEY='1'
...
union
SELECT loan_skey from #ib7temp
union
SELECT loan_skey from #ib6temp
union 
...
select * from #tmpRepairPopulation ;
--where loan_skey=100756;

----HCI tab
select * 
from #tmpRepairPopulation
where workflow_task_description in ('Receipt of Repairs Contract','Receipt of Contractor W-9'
,'Repair Rider - Repairs Completion Due','Receipt of Property Inspection Report for Repairs')
...
,'RSA Repair Inquiry' as CALLREASON,

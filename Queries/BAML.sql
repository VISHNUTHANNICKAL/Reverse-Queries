...
IF OBJECT_ID('tempdb..#tempBaml') IS NOT NULL
    DROP TABLE #tempBaml;
...
delete from #tempBaml where loan_skey not in(
select  loan_skey from
reversequest.rms.v_Alert where alert_type_description = 'BAML- Death Cert Needed' and alert_status_description = 'Active'
);

delete from #tempBaml
where loan_skey in
...
update #tempBaml set [Home Phone #]='',[Cell Phone #]=''
where [Contact Type] = 'Attorney';

select * from #tempBaml;

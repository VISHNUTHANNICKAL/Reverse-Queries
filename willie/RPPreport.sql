...
IF OBJECT_ID('tempdb..#tempTbl') IS NOT NULL
	...
	-- d.tots_balance,
	f.original_transfer_property_charge,f.current_transfer_property_charge_balance
,f.property_charge_pre_due_and_payable_balance,f.corporate_advance_S305_flood_insurance_balance,
f.corporate_advance_S305_hazard_insurance_balance,f.corporate_advance_S305_taxes_balance,
f.corporate_advance_S305_ground_rent_balance,f.corporate_advance_S305_HOA_balance,f.tots_balance
...
into #tempTbl
 ...
 left join rms.v_MonthlyLoanDefaultSummary f on b.loan_skey=f.loan_skey and f.created_date >=  DATEADD(m,-1,DATEADD(mm, DATEDIFF(m,0,GETDATE()), 0))
...
where 
 ...
 group by  b.loan_skey,c.loan_status_description,c.fha_case_number,c.investor_name,c.servicer_name,
	d.due_and_payable_date ,
	 a.schedule_date ,
	 --d.tots_balance,
	 d.default_reason_description,d.default_date,f.original_transfer_property_charge,f.current_transfer_property_charge_balance
,f.property_charge_pre_due_and_payable_balance,f.corporate_advance_S305_flood_insurance_balance,
f.corporate_advance_S305_hazard_insurance_balance,f.corporate_advance_S305_taxes_balance,
f.corporate_advance_S305_ground_rent_balance,f.corporate_advance_S305_HOA_balance,f.tots_balance
...
IF OBJECT_ID('tempdb..#tempTbl3') IS NOT NULL
  ...
  ,(a.[Repayment Plan Term]-b.[Complete date]) as '# Repayments Remaining' 
   ...
   ,g.[Complete Date Servicer Notified of Decision]
   into #tempTbl3
	from(  
	 select 
		b.loan_skey,
...
left join (
select 
		b.loan_skey,max(a.complete_date) as 'Complete Date Servicer Notified of Decision'
  FROM [ReverseQuest].[rms].[v_WorkflowTaskActivity] a
  join
  [ReverseQuest].[rms].[v_WorkflowInstance] b
  on a.workflow_instance_skey=b.workflow_instance_skey
   ...
   and a.workflow_task_description='Servicer Notified of Decision'
   and a.complete_date is not null
   and b.status_description='Active'
   and a.status_description='Active'
   group by b.loan_skey
...
IF OBJECT_ID('tempdb..#tempTbl4') IS NOT NULL
 ...
 ,d.[HUD Decision - Approved Complete Date],e.[HUD Decision - Denied Complete Date],f.extension_expiration_date
 into #tempTbl4 
 from 
 (
 select b.loan_skey
...
left join(
select period_description,created_date,created_by,loan_skey,extension_expiration_date
,tots_balance from rms.v_MonthlyLoanDefaultSummary
where loan_skey in(select b.loan_skey
	--,b.workflow_instance_skey
	--,b.workflow_type_description
	--,b.workflow_manager
	--,b.status_description
	--,a.workflow_task_description
	--,a.complete_date as 'Extension Expiration Date'
	from   [ReverseQuest].[rms].[v_WorkflowTaskActivity] a	left join
	 [ReverseQuest].[rms].[v_WorkflowInstance] b
	 on a.workflow_instance_skey=b.workflow_instance_skey
	 where --b.loan_skey=1930 and
	  b.workflow_type_description ='Extension - Property Charge Loss Mitigation'
	--and a.workflow_task_description='Extension - Property Charge Loss Mitigation'
	and b.status_description='Active'
  and a.status_description='Active'
	and a.complete_date is not null
	--order by loan_skey
...
a.[Remaining Amount],
a.[Original Transfer Property Charge],
a.[Transfer Property Charge],
a.[Property Charge Pre Due and Payable Balance],
a.[Corporate Advance S305 Flood Insurance Balance],
a.[Corporate Advance S305 Hazard Insurance Balance],
a.[Corporate Advance S305 Taxes Balance],
a.[Corporate Advance S305 Ground Rent Balance],
a.[Corporate Advance S305 HOA Balance],
--a.[New Tots Balance] as 'TOTS Balance',
case when( a.[Corporate Advance S305 Flood Insurance Balance] > 0 or a.[Corporate Advance S305 Hazard Insurance Balance] > 0 )
and a.[Corporate Advance S305 Taxes Balance] = 0 and a.[Corporate Advance S305 Ground Rent Balance] = 0 
and a.[Corporate Advance S305 HOA Balance] = 0
then 'Insurance'
when (a.[Corporate Advance S305 Flood Insurance Balance] > 0 or a.[Corporate Advance S305 Hazard Insurance Balance] > 0) 
and a.[Corporate Advance S305 Taxes Balance] > 0 and a.[Corporate Advance S305 Ground Rent Balance] = 0 
and a.[Corporate Advance S305 HOA Balance] = 0 then 'Insurance and Tax'
when (a.[Corporate Advance S305 Flood Insurance Balance] > 0 or a.[Corporate Advance S305 Hazard Insurance Balance] > 0) 
and a.[Corporate Advance S305 Taxes Balance] > 0 and a.[Corporate Advance S305 Ground Rent Balance] > 0 
and a.[Corporate Advance S305 HOA Balance] = 0
then 'Insurance, Tax and Ground'
when (a.[Corporate Advance S305 Flood Insurance Balance] > 0 or a.[Corporate Advance S305 Hazard Insurance Balance] > 0) 
and a.[Corporate Advance S305 Taxes Balance] > 0 and a.[Corporate Advance S305 Ground Rent Balance] > 0 
and a.[Corporate Advance S305 HOA Balance] > 0
then 'Insurance, Tax, Ground and HOA'
when (a.[Corporate Advance S305 Flood Insurance Balance] = 0 or a.[Corporate Advance S305 Hazard Insurance Balance] = 0) 
and a.[Corporate Advance S305 Taxes Balance] > 0 and a.[Corporate Advance S305 Ground Rent Balance] = 0 
and a.[Corporate Advance S305 HOA Balance] = 0 then 'Tax'
when (a.[Corporate Advance S305 Flood Insurance Balance] = 0 or a.[Corporate Advance S305 Hazard Insurance Balance] = 0) 
and a.[Corporate Advance S305 Taxes Balance] > 0 and a.[Corporate Advance S305 Ground Rent Balance] > 0 
and a.[Corporate Advance S305 HOA Balance] = 0 then 'Tax and Ground'
when (a.[Corporate Advance S305 Flood Insurance Balance] = 0 or a.[Corporate Advance S305 Hazard Insurance Balance] = 0) 
and a.[Corporate Advance S305 Taxes Balance] > 0 and a.[Corporate Advance S305 Ground Rent Balance] > 0 
and a.[Corporate Advance S305 HOA Balance] > 0 then 'Tax, Ground and HOA'
when (a.[Corporate Advance S305 Flood Insurance Balance] = 0 or a.[Corporate Advance S305 Hazard Insurance Balance] = 0) 
and a.[Corporate Advance S305 Taxes Balance] = 0 and a.[Corporate Advance S305 Ground Rent Balance] > 0 
and a.[Corporate Advance S305 HOA Balance] = 0 then 'Ground'
when (a.[Corporate Advance S305 Flood Insurance Balance] = 0 or a.[Corporate Advance S305 Hazard Insurance Balance] = 0) 
and a.[Corporate Advance S305 Taxes Balance] = 0 and a.[Corporate Advance S305 Ground Rent Balance] > 0 
and a.[Corporate Advance S305 HOA Balance] > 0 then 'Ground and HOA'
when (a.[Corporate Advance S305 Flood Insurance Balance] = 0 or a.[Corporate Advance S305 Hazard Insurance Balance] = 0) 
and a.[Corporate Advance S305 Taxes Balance] = 0 and a.[Corporate Advance S305 Ground Rent Balance] = 0 
and a.[Corporate Advance S305 HOA Balance] > 0 then 'HOA'
else 'NA'
end as "Disbursement Type",
a.[TOTS Balance],
a.[Default Reason],
a.[Default Date],
a.[Due & Payable Date],
...
a.[Extension Expiration Date],
a.[Complete Date Servicer Notified of Decision],
...
,coalesce(b.[Orig. Repayment Amt],0) as 'Orig. Repayment Amt'
,case when  b.[Last Repayment Received Date] is NULL then b.[Orig. Repayment Amt] else coalesce(b.[Remaining Amount],0) end as 'Remaining Amount'
,coalesce(a.original_transfer_property_charge,0) as 'Original Transfer Property Charge'
,coalesce(a.current_transfer_property_charge_balance,0) as 'Transfer Property Charge'
,convert(int,coalesce(a.property_charge_pre_due_and_payable_balance,0)) as 'Property Charge Pre Due and Payable Balance'
,convert(int,coalesce(a.corporate_advance_S305_flood_insurance_balance,0)) as 'Corporate Advance S305 Flood Insurance Balance'
,convert(int,coalesce(a.corporate_advance_S305_hazard_insurance_balance,0)) as 'Corporate Advance S305 Hazard Insurance Balance'
,convert(int,coalesce(a.corporate_advance_S305_taxes_balance,0)) as 'Corporate Advance S305 Taxes Balance'
,convert(int,coalesce(a.corporate_advance_S305_ground_rent_balance,0)) as 'Corporate Advance S305 Ground Rent Balance'
,convert(int,coalesce(a.corporate_advance_S305_HOA_balance,0)) as 'Corporate Advance S305 HOA Balance'
--,round(a.tots_balance,2) as 'New Tots Balance'
,round(a.Tots,2) as 'TOTS Balance'
,case when a.loan_status_description='FORECLOSURE' then e.default_reason_description else  d.default_reason_description end  as 'Default Reason',
...
,CONVERT(varchar,c.extension_expiration_date,23) as 'Extension Expiration Date'
,CONVERT(varchar,b.[Complete Date Servicer Notified of Decision],23) as 'Complete Date Servicer Notified of Decision'

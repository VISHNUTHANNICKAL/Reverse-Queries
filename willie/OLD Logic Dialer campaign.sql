...
where alert_status_description='Active') a where sn=1
)d
...
--select * from #finalDefault where investor_name='BAML' order by loan_skey
...
--select * from #finalDefault1 where loan_skey in(select loan_skey from #limitedattemptstaterules)
...
--left join #rtemp10 c on a.LOANSKEY=c.loan_skey
left join #rtemp11 d on a.LOANSKEY=d.loan_skey
left join #rtemp12 e on a.LOANSKEY=e.loan_skey
...
where CALLREASON in (
'Call RequestActive Occupancy','Call Required  Repairs',
'Call Request  Proof of Ins','Call RequestClaim Assignments',
'Call RequestProof of Paid Tax'
)
order by PRIORITYIBOB asc;
...
case when e.LMPRIORITY is null then 1 else e.LMPRIORITY end as 'LMPRIORITY'
 ...
 left join #rtemp12 e on x.LOANSKEY=e.loan_skey
